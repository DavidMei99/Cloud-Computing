...
aws ec2 run-instances ${PREAMBLE} --image-id ${AMI_ID} --count ${INSTANCES_COUNT} --iam-instance-profile Name=${INSTANCE_PROFILE_NAME} --instance-type ${INSTANCE_TYPE} --key-name ${KEY_NAME} --security-group-ids ${SEC_GROUP_ID} --subnet-id ${SN_ID_PUBLIC} \
